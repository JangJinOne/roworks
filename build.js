...
function buildHTML() {
  ...
  // 환경변수에서 따옴표 제거
  const apiKey = env.ANTHROPIC_API_KEY.replace(/^["']|["']$/g, '');
  const discordClientId = (env.DISCORD_CLIENT_ID || '1431242474768044070').replace(/^["']|["']$/g, '');
  ...
  // 환경변수 주입 (따옴표 없이)
  html = html.replace(/VITE_ANTHROPIC_API_KEY/g, apiKey);
  html = html.replace(/VITE_DISCORD_CLIENT_ID/g, discordClientId);
